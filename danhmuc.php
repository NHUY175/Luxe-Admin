        ...
        <div class="main-cate-content--wrapper">
                    ...
                    <form action="?opt=add_dmsp" method="post" enctype="multipart/form-data">
                        <div class="info-cate-add__inner">
                            <div class="info-cate-add__inner--ten-input">
                                <label for="ten_danhmuc" class="form-DMSP-label">Tên danh mục <span
                            ...
                            <div class="info-cate-add__inner--iamge">
                                <label for="HinhAnhDMSP" class="form-DMSP-label">Hình ảnh <span
                                        style="color:red">*</span></label>
            ...
            <table class="product-list">
                ...
                <tbody>
                    ...
                    function view_dmsp()
                    {
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<a href='sua_dmsp.php?id=" . $row["ma_danh_muc"] . "'><img src='./icon/sanpham-edit.svg' alt='Sửa' /></a>";
              ...
              <img src='./icon/sanpham-delete.svg' alt='Xóa' /></a>";
                        ...
                        if (isset($_POST["dieukien"]) && isset($_POST["giatri"])) { // Kiểm tra xem có dữ liệu POST được gửi lên không
                            ...
                            if (mysqli_num_rows($rs) > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($rs)) {
                                    ...
                                    echo "<td>
                    <div class='action'>
                        <a href='./sua_dmsp.php?id='><img src='./icon/sanpham-edit.svg' alt='Sửa' /></a>
                    ...
                    //     // Lưu giữ các giá trị đã nhập vào session
                    //     $_SESSION['ten_danhmuc'] = $_ten_danh_muc;
                    ...
                    //       $_hinh_anh_danh_muc = $_FILES["hinhanh"]["name"];
                    //       $_tmp_hinh_anh_danh_muc = $_FILES["hinhanh"]["tmp_name"];
                    //       $_target_file = "./img/" . basename($_hinh_anh_danh_muc);
                    
                    //       // Kiểm tra loại file
                    //       $_imageFileType = strtolower(pathinfo($_target_file, PATHINFO_EXTENSION));
                    //       $allowed_types = array('jpg', 'jpeg', 'png', 'gif');
                    //       if (!in_array($_imageFileType, $allowed_types)) {
                    //         echo "<script>alert('Chỉ chấp nhận các loại file JPG, JPEG, PNG và GIF.');</script>";
                    //         // Đánh dấu thông báo lỗi
                    //         $error = true;
                    //       }
                    ...
                    //       if (!move_uploaded_file($_tmp_hinh_anh_danh_muc, $_target_file)) {
                    //         echo "<script>alert('Đã xảy ra lỗi khi tải lên hình ảnh.');</script>";
                    //         // Đánh dấu thông báo lỗi
                    //         $error = true;
                    //       }
                    ...
                    //     // Nếu không có lỗi, tiếp tục thêm vào CSDL
                    //     if (!$error) {
                    //       // Thực hiện truy vấn thêm vào CSDL
                    //       $_sql = "INSERT INTO tbl_danhmuc (ten_danh_muc, hinh_anh_danh_muc) VALUES ('$_ten_danh_muc', '$_hinh_anh_danh_muc')";
                    //       $_result = chayTruyVanKhongTraVeDL($link, $_sql);
                    //       if ($_result) {
                    //         //Kiểm tra insert
                    ...
                    // // Kiểm tra nếu có phương thức POST được gửi đi
                    // if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    //   // Kiểm tra nếu biểu mẫu được gửi để thêm một danh mục sản phẩm mới
                    //   if (isset($_POST["ten_danhmuc"])) {
                    //     // Khởi tạo biến
                    //     $_ten_danh_muc = $_POST["ten_danhmuc"];
                    //     $_hinh_anh_danh_muc = ''; // Khởi tạo biến hình ảnh
                    
                    //     // Kiểm tra xem một hình ảnh đã được tải lên hay chưa
                    //     if (isset($_FILES["hinhanh"]) && $_FILES["hinhanh"]["error"] == 0) {
                    //       // Lưu trữ hình ảnh đã tải lên
                    //       $_tmp_hinh_anh_danh_muc = $_FILES["hinhanh"]["tmp_name"];
                    //       $_target_file = "./img/" . basename($_FILES["hinhanh"]["name"]);
                    ...
                    //       $_hinh_anh_danh_muc = basename($_FILES["hinhanh"]["name"]); // Lưu tên hình ảnh
                    //     }
                    
                    //     // Chèn danh mục sản phẩm mới vào cơ sở dữ liệu
                    //     $link = null; // Khởi tạo kết nối cơ sở dữ liệu
                    //     taoKetNoi($link); // Kết nối đến cơ sở dữ liệu
                    //     $_sql = "INSERT INTO tbl_danhmuc (ten_danh_muc, hinh_anh_danh_muc) VALUES ('$_ten_danh_muc', '$_hinh_anh_danh_muc')";
                    //     $_result = chayTruyVanKhongTraVeDL($link, $_sql); // Thực thi truy vấn
                    
                    //     // Kiểm tra xem truy vấn có thành công không
                    //     if ($_result) {
                    //       echo "<script>alert('Thêm danh mục sản phẩm thành công');</script>";
                    //       header("Location: danhmuc.php?opt=view_dmsp");
                    //       exit(); // Thoát khỏi mã
                    //     } else {
                    //       echo "<script>alert('Thêm danh mục sản phẩm thất bại');</script>";
                    //       header("Location: danhmuc.php?opt=view_dmsp");
                    ...
                    // // Nếu mã chạm đến điểm này, có nghĩa là có một vấn đề với cấu trúc mã
                    // echo "<script>alert('Có một vấn đề với cấu trúc mã.');</script>";
                    // exit(); // Thoát khỏi mã
                    // Kết nối CSDL
                    //tùy chọn
                    if (isset($_GET["opt"])) {
